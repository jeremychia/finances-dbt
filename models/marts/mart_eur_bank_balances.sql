with
    ...
    agg_local_amounts as (
        select
            ...
            sum(eur_amount) as total_eur_amount_spot_rate
        from {{ ref("mart_eur_bank_transactions") }}
        group by all
    ),
    ...
    cross_join as (
        select
            ...
            agg_local_amounts.total_eur_amount_spot_rate
        from dates
    ...
    calculate_balance as (
        select
            ...
            sum(
                case
                    when local_date >= transaction_date then total_eur_amount_spot_rate
                end
            ) as eur_spot_rate
        from cross_join
        group by all
    ...
    translate_to_eur as (
        select
            ...
            case
                when calculate_balance.local_currency = 'EUR'
                then 1
            ...
            round(calculate_balance.eur_spot_rate, 2) as eur_spot_rate
    ...
    calculate_fx_gain_loss as (
        select
            *,
            round(
                coalesce(eur_amount, 0) - coalesce(eur_spot_rate, 0), 2
            ) as eur_fx_gain_loss
        from translate_to_eur
    )
